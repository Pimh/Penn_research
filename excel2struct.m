function cell = excel2struct(fname,tvec,varargin)
...
for i= 1:ncell
    ...
    if (area > area_cutoff) & (is_in_intbound)
        ...
        cell(j)= struct('Time',tvec,'cellID',data(cur_row,1),'Area',data(cur_row,3),...
            'Intensity',data(cur_row,4), 'Norm_intensity',[]);
        
        % Calculate normalized intensity
        base = min(cell(j).Intensity(1:10));
        delta_int = (cell(j).Intensity-base)/base;
